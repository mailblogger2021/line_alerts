...
two_line_count  = 2 #3
time_frame = "day"
alert_file_name = f"two_line_alerts_{time_frame}.xlsx"
data_store_file_name = f"two_line_data_store_{time_frame}.json"
...
def generate_url(rows, time_frame, is_history_starting_from=False, is_add_indicator=True):
    stock_name = rows['STOCK NAME']
    os.makedirs(f"stock_historical_data/two_line_{time_frame}", exist_ok=True)
    global data_store
    try:
        if not (time_frame in data_store and stock_name in data_store[time_frame]):
            ...
            with requests.Session() as session:
                ...
                logging.info(f"is_history_starting_from - {is_history_starting_from}")
                candles = kite.get_kite_url(session, rows, time_frame, is_history_starting_from, is_add_indicator)
            ...
            if(is_history_starting_from):
                ...
                number_of_calls = 2*kite.maximum_candle_limit[time_frame]
        ...
        for combination in leatest_combinations:
            ...
            if(is_line):
                ...
                row_to_append = pd.DataFrame({
                    ...
                    'date3': [df.iloc[alert.index[2]].Datetime] if two_line_count>2 else 0,
                    'row3': [alert.index[2]] if two_line_count>2 else 0,
                    'value3': [alert.iloc[2, 0]] if two_line_count>2 else 0,
...
def save_files():
    global alerts,data_store
    try:
        alerts.drop_duplicates(subset=['stockname','date1', 'row1', 'value1', 'date2', 'row2', 'value2', 'date3', 'row3', 'value3', 'buyORsell', 'slope', 'intercept','window_size','percentage_value','two_line_count'], keep='last', inplace=True)
        alerts.to_excel(alert_file_name,index=False)
...
if __name__=="__main__":
    try:
        ...
        for index in range(0,len(stock_data)+10):
            threads = []
            for index_j in range(index,index+10):
                row = stock_data.iloc[index_j]
